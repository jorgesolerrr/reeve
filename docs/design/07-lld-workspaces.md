...
2. **Kill process-group orphans**: the run registry (08) tracks each Run's process group; even after the main process exited, children may survive (agent-started dev servers, MCP servers). Any survivor in the group is killed before touching disk. *(Refined by 08-lld-runs: a Run's group now dies with its Run at exit — this step stands as belt-and-braces over whatever the registry still knows.)* Reeve kills only what it spawned — a foreign handle (the user's editor open in the worktree) is never touched; it will exhaust the retries and be named in the final error.
